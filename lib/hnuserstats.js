...
var hnuserstats = function()
{
	this.get_data_and_stats = function(author, callback)
	{
		...
		async.parallel({
			...
			userinfo: function(callback)
			{
				...
				{
					saved_results.userinfo = results;
	...
	// this runs synchronously, stores statistics in data
	// TODO: add histograms?
	var _get_stats = function(data)
	{
		...
		data.ly_story_karma = 0;
		data.ly_story_count = 0;
		data.ly_comment_karma = 0;
		data.ly_comment_count = 0;
		data.ly_comment_toplevel_count = 0;
		data.ly_comment_toplevel_karma = 0;
		...
		for (i = 0; i < results.length; i++)
		{
			for (j = 0; j < results[i]._tags.length; j++)
			{
				...
				if (results[i]._tags[j] === "story")
				{
					...
					if (ly)
					{
						data.ly_story_karma += results[i].points - 1;
						data.ly_story_count++;
					}
					found_hit_type = true;					
					break;
				} else if (results[i]._tags[j] === "comment")
				{
					...
					if (ly)
					{
						data.ly_comment_karma += results[i].points - 1;
						data.ly_comment_count++;
					}
					found_hit_type = true;
					if ((results[i].parent_id) === (results[i].story_id))
					{
						...
						if (ly)
						{
							data.ly_comment_toplevel_count++;
							data.ly_comment_toplevel_karma += results[i].points - 1;
						}
					}
					break;
				}
...
{
	// TODO: clean up list of columns, etc. (should be stats, not data); also look at json output
